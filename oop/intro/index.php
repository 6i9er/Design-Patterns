...
//echo TEST;
//echo test;
//echo BattleManager::TYPE_NORMAL;
$container = new Container($configuration);
$shipLoader = $container->getShipLoader();
$ships = $shipLoader->getShips();
$rebelShip = new RebelShip("new Ship  ");
$brokenShip = new BrokenShip("new Broken");
$brokenShip->setStrength("12");
        ...
        <div>
            <form method="POST" action="battle.php">
                ...
                <select class="center-block form-control btn drp-dwn-width btn-default dropdown-toggle" name="ship1_id">
                    ...
                    <?php foreach ($ships as  $ship): ?>
                        <?php if($ship->isRepaired()): ?>
                        <option value="<?php echo $ship->getId(); ?>"><?php echo $ship->getNameAndSpaces(true); ?></option>
                            <?php endif ?>
                    <?php endforeach; ?>
                </select>
                <br>
                ...
                <p class="text-center">Battle Type</p>
                <select class="center-block form-control btn drp-dwn-width btn-default dropdown-toggle" name="battle_type">
                    <option value="">Choose a Battle Type</option>
                    <?php foreach ($battleTypes as $battleType => $name): ?>
                            <option value="<?php echo $battleType; ?>"><?php echo $name; ?></option>
                    <?php endforeach;; ?>
                </select>
                <br>
